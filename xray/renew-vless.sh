...
if update_expiry "$user" "$current_exp" "$new_exp"; then
    ...
    if systemctl restart xray > /dev/null 2>&1; then
        ...
        if [[ -f "/home/vps/public_html/vless-$user.txt" ]]; then
            sed -i "s/Expired On  : $current_exp/Expired On  : $new_exp/" "/home/vps/public_html/vless-$user.txt" 2>/dev/null
            sed -i "s/Expiry: $current_exp/Expiry: $new_exp/" "/home/vps/public_html/vless-$user.txt" 2>/dev/null
            sed -i "s/# Generated: .*/# Generated: $(date)/" "/home/vps/public_html/vless-$user.txt" 2>/dev/null
        fi
        
        # Update JSON config file if exists
        if [[ -f "/home/vps/public_html/vless-$user.json" ]]; then
            sed -i "s/\"expiry\": \"$current_exp\"/\"expiry\": \"$new_exp\"/" "/home/vps/public_html/vless-$user.json" 2>/dev/null
        fi
        ...
        # Log the renewal
        echo "$(date): Renewed VLess account $user from $current_exp to $new_exp (+$masaaktif days)" >> /var/log/renew-vless.log 2>/dev/null
