...
if [[ ${NUMBER_OF_CLIENTS} == '0' ]]; then
    clear
    echo -e "${red}=========================================${nc}"
    echo -e "${blue}           Renew Trojan           ${nc}"
    echo -e "${red}=========================================${nc}"
    echo ""
    echo -e "${yellow}You have no existing Trojan clients!${nc}"
    echo ""
    echo -e "${red}=========================================${nc}"
    echo ""
...
if sed -i "s/^#! $user $current_exp/#! $user $new_exp/" /usr/local/etc/xray/config.json 2>/dev/null; then
    ...
    if systemctl restart xray > /dev/null 2>&1; then
        ...
        if [[ -f "/home/vps/public_html/trojan-$user.txt" ]]; then
            sed -i "s/Expired On  : $current_exp/Expired On  : $new_exp/" "/home/vps/public_html/trojan-$user.txt" 2>/dev/null
            sed -i "s/Expiry: $current_exp/Expiry: $new_exp/" "/home/vps/public_html/trojan-$user.txt" 2>/dev/null
        fi
        
        # Display success message
        clear
        echo -e "${red}=========================================${nc}"
        echo -e "${green} Trojan Account Successfully Renewed ${nc}"
        echo -e "${red}=========================================${nc}"
        echo ""
