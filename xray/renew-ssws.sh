#!/bin/bash
# ==========================================
# Renew Shadowsocks Account - FIXED VERSION
# ==========================================
...
if update_expiry "$user" "$current_exp" "$new_exp"; then
    ...
    if systemctl restart xray; then
        ...
        if systemctl is-active --quiet xray; then
            ...
            clear
            echo -e "${red}=========================================${nc}"
            echo -e "${green}      SHADOWSOCKS ACCOUNT RENEWED     ${nc}"
            echo -e "${red}=========================================${nc}"
            echo ""
            ...
            # Log the renewal
            echo "$(date): Renewed Shadowsocks account '$user' ($services) from '$current_exp' to '$new_exp' (+$masaaktif days)" >> /var/log/renew-shadowsocks.log 2>/dev/null
        else
