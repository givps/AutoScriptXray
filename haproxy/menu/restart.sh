...
display_menu() {
    ...
    echo -e " ${cyan}3${nc}  Restart Nginx"
    echo -e " ${cyan}4${nc}  Restart Badvpn"
    echo -e " ${cyan}5${nc}  Restart Xray"
    echo -e " ${cyan}6${nc}  Restart Haproxy Websocket"
    echo -e " ${cyan}7${nc}  Show Service Status"
    echo -e ""
...
show_service_status() {
    ...
    check_service_status "haproxy" "Haproxy Service" 
    check_service_status "ws-proxy" "WebSocket"
    echo -e "${red}-----------------------------------------${nc}"
    echo -e ""
}
...
restart() {
    ...
    while true; do
        ...
        case $Restart in
            ...
            6)
                display_header
                restart_service "haproxy" "systemctl restart haproxy.service" "Haproxy Service"
                restart_service "ws-proxy" "systemctl restart ws-proxy.service" "WebSocket"
                show_service_status
                wait_for_input
                ;;
            7)
